...
NIH_DIR = os.path.join(os.getenv('ML_DATA'), 'NIH')
NIH_LT_DIR = os.path.join(os.getenv('ML_DATA'), 'NIH-lt')
NIH_DARP_DIR = os.path.join(os.getenv('ML_DATA'), 'NIH-darp')
...
class NIH10(object):
  ...
  def load_raw_data(self):
    """Loads NIH10 raw data."""
    self.data_name = 'NIH10'
    (x_train, y_train) = data_util.load_tfrecord(
        os.path.join(NIH_DIR, 'NIH10-train.tfrecord'))
    (x_test, y_test) = data_util.load_tfrecord(
        os.path.join(NIH_DIR, 'NIH10-test.tfrecord'))
...
class OpenI(NIH10):
  ...
  def load_raw_data(self):
    """Loads OpenI raw data."""
    self.data_name = 'OpenI'
    (x_train, y_train) = data_util.load_tfrecord(
        os.path.join(NIH_DIR, 'OpenI-train.tfrecord'))
    (x_test, y_test) = data_util.load_tfrecord(
        os.path.join(NIH_DIR, 'OpenI-test.tfrecord'))
    self.x_train = x_train
    self.y_train = y_train
    self.x_test = x_test
    self.y_test = y_test
...
class NIH10LT(object):
  """NIH10 long-tail data loader."""

  def __init__(self, class_im_ratio):
    self.load_raw_data(class_im_ratio)

  def load_raw_data(self, class_im_ratio):
    """Loads NIH10 long-tail raw data."""
    self.data_name = 'NIH10lt@{}'.format(class_im_ratio)
    dir_path = os.path.join(NIH_LT_DIR,
                            'NIH-10-data-im-{}'.format(class_im_ratio))
...
class OpenILT(NIH10LT):
  """OpenI long-tail data loader."""

  def load_raw_data(self, class_im_ratio):
    """Loads OpenI long-tail raw data."""
    self.data_name = 'OpenIlt@{}'.format(class_im_ratio)
    dir_path = os.path.join(NIH_LT_DIR,
                            'NIH-100-data-im-{}'.format(class_im_ratio))
...
class NIH10LTDARP(NIH10LT):
  """NIH10 long-tail data loader following DARP setting.
  ...
  def load_raw_data(self, class_im_ratio):
    self.data_name = 'NIH10ltdarp{}'.format(class_im_ratio)
    data_shape = {'depth': 3, 'height': 32, 'width': 32}
    (x_train, y_train) = data_util.load_tfrecord(
        os.path.join(
            NIH_DARP_DIR,
            'NIH10-{}-train-labeled.tfrecords'.format(int(class_im_ratio))),
        is_raw=True,
        **data_shape)
    (x_unlab_test, y_unlab_test) = data_util.load_tfrecord(
        os.path.join(
            NIH_DARP_DIR,
            'NIH10-{}-train-unlabeled.tfrecords'.format(int(class_im_ratio))),
        is_raw=True,
        **data_shape)
    (x_test, y_test) = data_util.load_tfrecord(
        os.path.join(NIH_DARP_DIR, 'NIH10-test.tfrecords'),
        is_raw=True,
        **data_shape)
    self.x_train = x_train
    self.y_train = y_train
